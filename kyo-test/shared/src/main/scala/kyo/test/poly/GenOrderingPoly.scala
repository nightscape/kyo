...
object GenOrderingPoly:
    ...
    /** Constructs an instance of `GenOrderingPoly` using the specified `Gen` and `Ordering` instances, existentially hiding the underlying
      * type.
      */
    ...
    /** Derives an `Ordering[List[A]]` given an `Ordering[A]`.
      */
    implicit private def ListOrdering[A: Ordering]: Ordering[List[A]] =
        ...
        def loop[A: Ordering](left: List[A], right: List[A]): Int =
            (left, right) match
                ...
                case ((h1 :: t1), (h2 :: t2)) =>
                    ...
                    if compare == 0 then loop(t1, t2) else compare
    ...
    /** Derives an `Ordering[Vector[A]]` given an `Ordering[A]`.
      */
    implicit private def VectorOrdering[A: Ordering]: Ordering[Vector[A]] =
        (l, r) =>
            val j = l.length
            val k = r.length
            ...
            def loop(i: Int): Int =
                if i == j && i == k then 0
                    ...
                    if compare == 0 then loop(i + 1) else compare
