    ...
    def forallIterable[A](predicate: TestArrow[A, Boolean]): TestArrow[Iterable[A], Boolean] =
        TestArrow
            .make[Iterable[A], Boolean] { seq =>
                ...
                val elements = if failures.size == 1 then "element" else "elements"

                TestTrace.Node(
                    Result.succeed(failures.isEmpty),
                    M.pretty(failures.size) + M.text(s"$elements failed the predicate"),
                    children = if failures.isEmpty then None else Some(failures.reduce(_ && _))
                )
            }
    ...
    def existsIterable[A](predicate: TestArrow[A, Boolean]): TestArrow[Iterable[A], Boolean] =
        TestArrow
            .make[Iterable[A], Boolean] { seq =>
                ...
                val elements = if success.nonEmpty then "element" else "elements"

                val elementsSatisfiedPredicateMsg =
                    if result.exists(_.isDie) then
        ...
        TestArrow
            .make[A, Boolean] { a =>
                val result = (a, that) match
                    case (a: Array[?], that: Array[?]) => a.sameElements[Any](that)
                    case _                             => a == that
    ...
    def asCauseDie[E]: TestArrow[Cause[E], Throwable] =
        TestArrow
            .make[Cause[E], Throwable] {
                case cause if cause.panicOption.isDefined =>
                    TestTrace.succeed(cause.panicOption.get)
                case _ =>
    ...
    def asExitDie[E, A]: TestArrow[Exit[E, A], Throwable] =
        TestArrow
            .make[Exit[E, A], Throwable] {
                case Exit.Failure(cause) if cause.panicOption.isDefined =>
                    TestTrace.succeed(cause.panicOption.get)
                case Exit.Success(_) =>
