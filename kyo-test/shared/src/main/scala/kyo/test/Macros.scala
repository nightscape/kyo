/*
 * Copyright 2021-2024 John A. De Goes and the Kyo Contributors
 *
...
object Macros:
    ...
    )(assertion: Expr[Assertion[A]])(using Quotes): Expr[TestResult < Env[R] & Abort[E]] =
        import quotes.reflect.*
        val code          = Expr(showExpr(effect))
        val assertionCode = Expr(showExpr(assertion))
        '{ _root_.kyo.test.CompileVariants.assertKyoProxy($effect, $code, $assertionCode)($assertion) }
    end assertKyo_impl
